...
public class CampgroundCLI {
	...
	public Reservation handleSitesSelection(Campground ChoosenCampground, String arrivalDate, String departDate) {
		...
		if (popularSites.size() == 0) {
			...
			System.out.println("");
			System.out.printf("\n %-15s %-15s \t%-15s %-15s \t%-15s", "Site No.", "Max Occup.", "Accessible?",
					"RV Length", "Utility");
		...
		Long reservationid = jdbcReservationDAO.newReservation(reservation.getSiteid(), reservation.getName(),
	...
	public void retrieveReservation() {
		...
		if (numInput == reservationc.getReservationid()) {
			System.out.println("Hello " + reservationc.getName() + "! \n" + "You reserved siteid: "
					+ reservationc.getSiteid() + " for the dates of " + reservationc.getFromDate() + " to "
